---
name: pptx-figure
description: 根据MD文件生成源PPTX风格的学术绘图（浅色tint填充+黑1.5pt边框+黑粗体字体系）。支持燕尾页签泳道图、多栏技术板、分层架构图、总-分关系图四种版式原型，输出PowerPoint可导入的矢量SVG（可转换为原生形状）、PNG或Mermaid。
---
...
# 生成PPTX可导入的矢量SVG（推荐：无损缩放，可在PPT中“转换为形状”编辑）
python .claude/skills/pptx-figure/generate-figure.py content.md -o figure.svg

# 指定版式原型
python .claude/skills/pptx-figure/generate-figure.py content.md -o f.svg --type route         # 原型A 燕尾页签+泳道
python .claude/skills/pptx-figure/generate-figure.py content.md -o f.svg --type panel         # 原型B 多栏技术板
python .claude/skills/pptx-figure/generate-figure.py content.md -o f.svg --type architecture  # 原型C 分层架构
python .claude/skills/pptx-figure/generate-figure.py content.md -o f.svg --type overview      # 原型D 总-分关系
...
**输出格式由 `-o` 扩展名决定**：
- `.svg` — 手写矢量图（内联属性、无CSS/defs），**PowerPoint“插入>图片”可直接导入**，
  再“图形工具>转换为形状”即变成原生可编辑图形（圆角矩形/椭圆/文本框/连接线）。**无需 matplotlib。**
- `.png` — matplotlib 栅格图（需 `pip install matplotlib`）
- `.mermaid` — Mermaid 源码
...
S.set_backend('svg')                      # 'svg'矢量 或 'mpl'栅格(默认)
...
同一套组件代码对两个后端通用（`set_backend` 切换）。SVG后端零第三方依赖，PNG后端需 matplotlib。
...
1. 读取MD提取结构 → 2. 选版式原型 → 3. 应用组件库绘制 → 4. 按扩展名输出 SVG矢量 / PNG栅格 / Mermaid
