...
一个 [Claude Code](https://claude.com/claude-code) Skill：根据 Markdown 文件生成**学术项目申报书风格**的架构图、技术路线图、总-分关系图（PNG / Mermaid）。

风格体系逆向自某 41 页学术项目申报 PPT 图集——通过逐页渲染视觉分析 + OOXML 形状级数据提取，
归纳出完整的配色系统、组件词汇表、9 种布局原型与反面清单，再用 matplotlib 组件库参数化复现。
...
python .claude/skills/pptx-figure/generate-figure.py research.md -o figure.png --type route
...
├── templates/python/pptx_style_base.py     # matplotlib组件库
